...
function Hub(options) {
    ...
    this.listen = function () {
        ...
        // SO_REUSEADDR: 允许端口被复用, 避免 EADDRINUSE 导致收不到数据
        this.socket.bind({ port: options.port, exclusive: false });
    };
    ...
    this.onListening = function () {
        ...
        try {
            if (options.bind && options.bind !== '0.0.0.0') {
                ...
                // 0.0.0.0: 遍历所有非 loopback 接口加入组播, 避免容器默认接口不是 LAN 口导致收不到组播
                const os = require('os');
                ...
                for (const name of Object.keys(ifaces)) {
                    for (const addr of ifaces[name]) {
                        ...
                        if (addr.family === 'IPv4') {
                            try {
                                this.socket.addMembership('224.0.0.50', addr.address);
                                joined = true;
                                console.log('[hub] addMembership 224.0.0.50 on ' + name + '(' + addr.address + ')');
                            } catch (e) { /* 接口不支持组播则跳过 */ }
                        }
                    }
                }
                if (!joined) this.socket.addMembership('224.0.0.50');
            }
        } catch (err) {
    ...
    this.onMessage = function (msgBuffer, rinfo) {
        ...
        if (sensor) {
            ...
            if (msg.data && (msg.cmd === 'report' || msg.cmd.indexOf('_ack') !== -1)) {
                if (msg.cmd === 'write_ack') console.log('[hub] WRITE_ACK from ' + rinfo.address + ': ' + JSON.stringify(msg.data));
    ...
    this.getKey = function (ip) {
        ...
        try {
            ...
            console.log('[hub] getKey ip=' + ip + ' token=' + this.token[ip] + ' -> ' + crypted);
            return crypted;
        } catch (err) {
    ...
    this.sendMessage = function (message, ip) {
        ...
        console.log('[hub] SEND -> ' + (ip || '224.0.0.50') + ':9898 ' + json);
        this.socket.send(json, 0, json.length, 9898, ip || '224.0.0.50');
    };
