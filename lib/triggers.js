...
class TriggerEngine {
    ...
    constructor(hubGetter, gateways, rules, config) {
        ...
        this._heldByDoor = {};
        this._doorStates = {};
        this._doorStateAtTrigger = {}; // 触发时记录的门状态
    }
    ...
    _scheduleOff(rule) {
        ...
        this._timers[key] = setTimeout(() => {
            ...
            if (dg) {
                // 用触发时的门状态判断，而不是当前状态
                const doorWasClosed = this._doorStateAtTrigger[key] === false;
                if (doorWasClosed) {
                    // 触发时门关着 → 人在里面 → 等门开了再关
                    this._heldByDoor[key] = true;
                    console.log(`[trigger] ${rule.name}: 触发时门(${dg})关着, 保持亮灯等门开`);
                    delete this._timers[key];
                    delete this._doorStateAtTrigger[key];
                    return;
                }
            }
            // 触发时门开着 → 人只是路过 → 直接关
    ...
    onData(sid, data) {
        ...
        for (const rule of this._rules) {
            ...
            const dg = rule.doorGuard;
            if (dg) {
                this._doorStateAtTrigger[key] = this._doorStates[dg];
            }
            this._heldByDoor[key] = false;
            console.log(`[trigger] ${rule.name}: 命中 -> onValue=${rule.onValue}` + (dg ? `, 触发时门状态=${this._doorStates[dg]}` : ''));
