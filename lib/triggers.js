...
class TriggerEngine {
    ...
    _startTimer(rule, delay) {
        ...
        this._timers[key] = setTimeout(() => {
            ...
            console.log(`[trigger] ${rule.name}: 定时器到期 doorClosed=${this._doorClosed[key]} motionAfterClose=${this._motionAfterClose[key]}`);
    ...
    _scheduleOff(rule) {
        ...
        if (rule.doorGuard) {
            ...
            // 只在门未关(新触发周期)时重置 motionAfterClose，门关着时保留 onData 设置的值
            if (!this._doorClosed[key]) {
                this._motionAfterClose[key] = false;
            }
            console.log(`[trigger] ${rule.name}: _scheduleOff doorClosed=${this._doorClosed[key]} motionAfterClose=${this._motionAfterClose[key]} doorState=${this._doorStates[rule.doorGuard]}`);
        }
     ...
     */
    onDoor(sid, data) {
        console.log(`[trigger] onDoor: sid=${sid} hasTimer=${!!this._timers[this._ruleKey(this._rules[0])]} doorStates=${JSON.stringify(this._doorStates)}`);
